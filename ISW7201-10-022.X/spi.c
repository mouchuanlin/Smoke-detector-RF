...
//    APFCON0bits.SSSEL = 0;          // ~SS is always on pin RC6
    ...
    SSP1STAT = 0x00;
    SSP1CON1 = 0b00100000;//0x00;
    SSP1CON1bits.SSPM = 0b0000;     // SCK = F_osc/16
    
    
    SSP1CON1bits.CKP = 0;
    SSP1STATbits.CKE = 1;
    SSP1STATbits.SMP = 1;//0;
    
    SSP1CON2 = 0;
    SSP1ADD = 9;        // 100kHz clock speed with 4MHz clock
...
rfStatus_t cmdStrobe(uint8_t cmd)
{
    uint8_t rc;
    TRXEM_SPI_BEGIN();
//    while(MISO);
...
//  while(MISO);
  ...
  readValue = SPIWrite((uint8_t)(accessType|addrByte));
...
rfStatus_t trx16BitRegAccess(uint8_t accessType, uint8_t extAddr, uint8_t regAddr, uint8_t *data, uint8_t len)
{
  ...
  readValue = SPIWrite((uint8_t)(accessType|extAddr));
  SPIWrite(regAddr);
